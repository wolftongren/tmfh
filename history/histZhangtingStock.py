...
sql = "SELECT distinct code, name FROM `histZhangting1317`"
dfStocks = pd.read_sql(sql, conn)

print "reading zhangting history from zhangtingHistory..."
sql = "SELECT * FROM `histZhangting1317`"
...
dfResult.to_sql('histZhangtingStock1317', engine, index=False, if_exists='append')